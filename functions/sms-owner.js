exports.handler = async (context, event, callback) => {
    ...
    const deleteMessage = async (recordingSid) => {
        try {
          ...
          if (result === true) {
            client.messages.create({ to, from, body: "Message deleted." })
              .then((result) => {
            ...
            client.messages.create({ to, from, body: "Error occurred." })
              .then((result) => {
...
if (smsRes === 'delete') {
  ...
  if (!recording) {
    ...
    client.messages.create({ to, from, body: "Error occurred. No recording found." }).then(() => {
  ...
  client.messages.create({ to, from, body: "Error occurred. Please call in to your voicemail to complete your action." })
    .then((result) => {
      console.log(result.sid);
      return callback(null, '');
    })
